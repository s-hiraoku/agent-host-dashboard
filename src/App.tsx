      ...
      {contextSource && state.status === "ready" && state.entries.map((entry, entryIndex) => (
        <article className="repository-card" key={`${entry.repository.locator.host}/${entry.repository.locator.owner}/${entry.repository.locator.name}`}>
          ...
          <div className="repository-columns">
            ...
            <section aria-labelledby={`prs-${entryIndex}`}><div className="repository-list-heading"><h4 id={`prs-${entryIndex}`}>Open pull requests</h4><span>{entry.pullRequests.length}</span></div><ul>{entry.pullRequests.map(({ pullRequest, relationship }) => <li key={pullRequest.id}><a href={pullRequest.url} target="_blank" rel="noreferrer"><GitPullRequest aria-hidden="true" /><span className="mono">#{pullRequest.number}</span><span>{pullRequest.title}</span></a><span className={`pr-relationship relationship-${relationship}`}>{relationshipLabel[relationship]}</span><small>{pullRequest.draft ? "Draft" : pullRequest.state} · checks {pullRequest.checks} · review {pullRequest.review}</small></li>)}</ul>{!entry.pullRequests.length && <p className="repository-empty">No open pull requests in this page.</p>}</section>
          </div>
        </article>
      ))}
