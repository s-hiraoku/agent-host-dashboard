...
Authentication is an ephemeral, host-keyed callback evaluated immediately before a network request. Host validation runs first, so a credential is never requested for an unconfigured origin and GitHub Enterprise credentials can remain isolated from GitHub.com. The client never accepts a token in repository/domain objects, never persists it, and does not include upstream response bodies in structured errors. The Settings surface keeps the credential in a dedicated memory-only vault, clears its uncontrolled password field on submit, and destroys the vault on disconnect, reload, or a repository authentication failure. Operators should grant only the minimum read permissions required for repository metadata, Issues, and pull requests.
