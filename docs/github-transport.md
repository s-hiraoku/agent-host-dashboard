...
The client pins `X-GitHub-Api-Version: 2026-03-10`, sends the recommended `application/vnd.github+json` media type, follows at most three same-origin redirects, and rejects unconfigured hosts before requesting credentials. GitHub.com is configured by default. GitHub Enterprise requires an explicit host-to-HTTPS-API-endpoint mapping.
